...
void MouseDetector::printCenterCoordinates(cv::Mat image, cv::Point centerCoordinates){ 
	if (mouseDetected()) {
		...
		double scale = 0.5;
		int thickness = 1;
		cv::putText(image,coordinate,textOrg,fontface,scale,colorDelMouse,thickness);
	}
}
...
void MouseDetector::processCenterMarker(cv::Mat &image) {
	...
	for (itc = contoursCenter.begin(); itc != contoursCenter.end(); ++itc) {
		if (cv::contourArea(*itc) > minMarkerArea) {
			bMouseDetected = true;
			cv::rectangle(image,cv::boundingRect(cv::Mat(*itc)),colorDelMouse,2);
			cv::Moments mom = cv::moments(cv::Mat(*itc));
			coordenadasMouse = cv::Point(int(mom.m10 / mom.m00), int(mom.m01 / mom.m00));
			cv::circle(image, coordenadasMouse, 2, colorDelMouse, 2);
			printCenterCoordinates(image, coordenadasMouse);
		}
	}
}

void MouseDetector::processLeftMarker(cv::Mat &image) {
	cv::cvtColor(image,image,CV_BGR2HSV); //conversion de RGB a HSV
	...
	if (bMouseDetected) {
		for (itc = contoursLeft.begin(); itc != contoursLeft.end(); ++itc) {
			if (cv::contourArea(*itc) > minMarkerArea) {
				...
				cv::rectangle(image,cv::boundingRect(cv::Mat(*itc)),colorClickIzquiero,2);
			}
		}
	}
}
	...
	if (bMouseDetected && !bLeftClickDetected) {
		for (itc = contoursRight.begin(); itc != contoursRight.end(); ++itc) {
			if (cv::contourArea(*itc) > minMarkerArea) {
				...
				cv::rectangle(image,cv::boundingRect(cv::Mat(*itc)),redColor,2);
			}
		}
	}
}
...
void MouseDetector::displayMouseStatus(cv::Mat &image) {
	...
	int thickness = 2;
	if (mouseDetected()) {
		cv::putText(image,"Mouse Detectado",cv::Point(10,30),cv::FONT_HERSHEY_SIMPLEX,scale,colorDelMouse,thickness);
	} else {
		cv::putText(image,"No hay un Mouse Detectado",cv::Point(10,30),cv::FONT_HERSHEY_SIMPLEX,scale,colorDelMouse,thickness);
	}
	if (scrollDetected()) {
		if (getScrollSpeed() > 0)
			cv::putText(image,"Subiendo",cv::Point(10,70),cv::FONT_HERSHEY_SIMPLEX,scale,greenColor,thickness);
		else
			cv::putText(image,"Bajando",cv::Point(10,70),cv::FONT_HERSHEY_SIMPLEX,scale,greenColor,thickness);
	}
	if (getLeftClickStatus() == true) {
		cv::putText(image,"Click Izquierdo",cv::Point(10,70+40*scrollDetected()),cv::FONT_HERSHEY_SIMPLEX,scale,colorClickIzquiero,thickness);
	} else if (getRightClickStatus() == true) {
		cv::putText(image,"Click Derecho",cv::Point(10,70+40*scrollDetected()),cv::FONT_HERSHEY_SIMPLEX,scale,redColor,thickness);
	}
